...
function formatDate(inputDate) {
  ...
  const date = new Date(year, parseInt(month) - 1); // Month is 0-indexed in Date constructor
...
const InvoiceMonthly = (props) => {
  let data = props.data;
  if (props.people.length === 1) {
    data = props.people;
  } else {
    data = props.people.filter((person) => {
      return person.email === props.email;
    });
  }
  ...
  const TableHead = () => (
    <View style={{ width: "100%", flexDirection: "row", marginTop: 10 }}>
      <View style={[styles.theader, styles.theader2]}>
        <Text>{formatDate(props.month)}</Text>
      </View>
    </View>
  );
